...
3. 若 `docs/DES-002-R3-overall-migration-redesign.md` 的 `reviewStatus` 不是 `approved`，或 `automation/state/current.json` 的 `controlPlaneStatus` 不是 `ready`，立即停止，不修改业务代码。R3 保持 `In Progress` 只表示执行前置尚未完成，不代表评审未通过。
