...
1. 读只读计划 `automation/plan/migration-plan.json`、`automation/state/current.json` 和对应 `automation/runs/<runId>/task.json` Worker 清单；`docs/session-current.md` 只作本地治理参考。
2. **`workerStatus` 必须为 `ported_awaiting_verifier`**，否则 **立即停止**。
3. 若 `docs/DES-002-R3-overall-migration-redesign.md` 的 `reviewStatus` 不是 `approved`，或全局 `controlPlaneStatus` 不是 `ready`，立即停止；验证时还必须读取该 run 的 `planRevision`、`sliceId`，复核该切片 manifest/baseline 版本与 run 快照一致。
