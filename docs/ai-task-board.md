...
1. Phase 0 核心决策已 Done；**LC-011 In Progress**（`EovaExp` 14:30Z 通过；`SqlParse` 15:05Z 通过；Orchestrator 2026-08-29T15:21Z **未新认领**，`EovaExpParam` 清单已在故保留）。LC-001 已 Deferred。Worker draft PR `#5` 已代码级 port `EovaExpParam`（替换 stub），**待 Verifier**。禁止重 port `EovaExp`/`SqlParse`。Verifier 通过前不派再下一单元。
