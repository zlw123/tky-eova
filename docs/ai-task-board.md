...
1. Phase 0 核心决策已 Done；**LC-011 In Progress**（`EovaExp` 14:30Z 通过；`SqlParse` 15:05Z 通过；`EovaExpParam` 15:37Z 通过；`SqlCondition` 16:02Z 通过）。Verifier 2026-08-29T16:02Z 通过 PR `#7`，Worker 清单已清空，待 Orchestrator 派下一单元。LC-001 已 Deferred。禁止重 port `EovaExp`/`SqlParse`/`EovaExpParam`/`SqlCondition`。并行 Worker `bc-5e0fea14` 禁止再开 `SqlCondition` 单。`TableSource` 仍为 compile-stub，不得标整任务 Done。`EovaExpBuilder` 依赖 `ExpUtil`/`Db`，本轮不派。
