...
1. Phase 0 核心决策已 Done；**LC-011 In Progress**（`EovaExp` 14:30Z 通过；`SqlParse` 15:05Z 通过；`EovaExpParam` 由 Verifier 于 15:37Z **通过**；Worker 清单已清空，待 Orchestrator 派下一单元）。LC-001 已 Deferred。禁止重 port `EovaExp`/`SqlParse`/`EovaExpParam`。`TableSource` 仍为 compile-stub，不得标整任务 Done。
