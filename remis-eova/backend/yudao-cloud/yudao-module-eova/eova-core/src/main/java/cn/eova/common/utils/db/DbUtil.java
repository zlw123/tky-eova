...
/**
 * <p>ported from: cn.eova.common.utils.db.DbUtil
 * <br>source revision: meta-eova/eova 1b1d39e7350f7e031b216aad0399fc8cc55dce08
 * <br>本单元为逐行等价 port：文件体与旧实现逐字节一致，仅新增本追溯头。
 * <br><b>刻意保留的既有语义：</b>
 * <ol>
 *   <li>数据库工具（604 行）：建表 SQL 生成、数据导出、列类型映射、表/列枚举</li>
 *   <li>【已声明适配 1】Db.use(ds) -> EovaGateways.get(ds)；Record -> EovaRecord</li>
 *   <li>【已声明适配 2】用到的网关方法 queryColumn / queryStr / queryLong 均已按旧 jfinal DbPro</li>
 *   <li>    语义提供（queryColumn 的 "Only ONE COLUMN can be queried." 抛错分支见接口 javadoc）</li>
 *   <li>无 jfinal 内核依赖（hostDeps 仅 Db/Record/DbType/fastjson/x），故不涉及决策 3</li>
 * </ol>
 */
public class DbUtil {
    ...
    public static StringBuilder createOracleSql(String ds, String tableNamePattern) {
        ...
        for (String table : tables) {
            ...
            Object val = EovaGateways.get(ds).queryColumn(sql);
     ...
     * @param sb
     */
    public static void generateSql(String ds, String table, List<EovaRecord> list, String auto, StringBuilder sb) {
        ...
        for (EovaRecord r : list) {
            ...
            if (table.equals("eova_menu")) {
                ...
                String pcode = EovaGateways.get(Ds.EOVA).queryStr(findPCode, pid);
                // 迁移后自动按Code 更新关系
     ...
     * @param list
     */
    public static StringBuilder dataToSql(String ds, String table, List<EovaRecord> list) {
        String EP = DbDialect.getEscape(EovaDataSource.getDbType(ds));
    ...
    private static void appendSql(String EP, String ds, String table, String auto, StringBuilder sb, EovaRecord r) {
    ...
    public boolean isExist(String ds, String sql, Object... paras) {
        Long count = EovaGateways.get(ds).queryLong(sql, paras);
