...
import cn.eova.db.ModelSqlBuilder;

/**
 * <p>ported from: cn.eova.widget.grid.GridController
 * <br>source revision: meta-eova/eova 1b1d39e7350f7e031b216aad0399fc8cc55dce08
 * <br>本单元为逐行等价 port：文件体与旧实现逐字节一致，仅新增本追溯头。
 * <br><b>刻意保留的既有语义：</b>
 * <ol>
 *   <li>表格控制器（673 行）：查询/编辑/删除/导出，含 Db.tx(IAtom) 事务体多处</li>
 * </ol>
 */
/**
 * Grid组件
...
public class GridController extends BaseController {
    ...
    public void export() throws Exception {
        ...
        try {
            // count 过滤 order by
            sql = ModelSqlBuilder.replaceOrderBy(sql);
            Long count = EovaGateways.get(object.getDs()).queryLong("select count(*) " + sql, parmList.toArray());
...
//		}
        ...
        EovaPage<EovaRecord> page = EovaGateways.get(object.getDs()).paginate(pageNumber, pageSize, select, sql, parmList.toArray());
...
//		}
        ...
        StringBuilder sb = new StringBuilder(String.format(ui, page.getTotalRow(), LegacyJsonKit.toJson(page.getList())));
...
//		}
    ...
    public void add() throws Exception {
        ...
        final List<EovaRecord> records = getRecordsByJson(json, object.getFields(), object.getPk(), object.getDs());

        intercept = TemplateUtil.initMetaObjectIntercept(object.getBizIntercept());
        // 事务
        EovaGateways.get(object.getDs()).tx(new LegacyIAtom() {
            @Override
            public boolean run() throws SQLException {
                try {
                    for (EovaRecord record : records) {
                        ...
                        EovaGateways.get(object.getDs()).save(object.getTable(), object.getPk(), record);
                        // 新增后置任务
                        if (intercept != null) {
                ...
                try {
                    for (EovaRecord record : records) {
                        ...
                        if (!x.isEmpty(object.getTable())) {
                            ...
                            if (isDel) {
                                // 删除数据
                                EovaGateways.get(object.getDs()).deleteById(object.getTable(), pk, object.buildPkValue(pkValue));
                            } else {
                                // 隐藏数据
                                ...
                                EovaGateways.get(object.getDs()).update(sql, pkValue);
                            }
                        } else {
            ...
            public boolean run() throws SQLException {
                try {
                    for (EovaRecord record : records) {
                        ...
                        EovaGateways.get(object.getDs()).update(object.getTable(), object.getPk(), record);
                        // 修改后置任务
                        if (intercept != null) {
     ...
     * @return
     */
    private static List<EovaRecord> getRecordsByJson(String json, List<MetaField> items, String pkName, String ds) {
    ...
    public void updateCell() throws Exception {
        ...
        EovaRecord cell = new EovaRecord();
        cell.set("pk", pk);
        ...
        try {
            ...
            if (object.isView() && intercept != null) {
                ...
                EovaGateways.get(object.getDs()).update(String.format("update %s set %s = ? where %s = ?", object.getTable(), field, object.getPk()), fieldValue, pkValue);
            }
