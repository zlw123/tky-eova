...
/**
 * <p>ported from: cn.eova.common.utils.db.DsUtil
 * <br>source revision: meta-eova/eova 1b1d39e7350f7e031b216aad0399fc8cc55dce08
 * <br>本单元为逐行等价 port：文件体与旧实现逐字节一致，仅新增本追溯头。
 * <br><b>刻意保留的既有语义：</b>
 * <ol>
 *   <li>数据源与表结构自省工具（433 行）：getConnection / getDatabaseMetaData / 表与列枚举</li>
 *   <li>【已声明适配 1】DbKit.getConfig(ds) -> EovaGateways.find(ds)（精确查找，未注册返回 null）</li>
 *   <li>    + Config.getDataSource() -> EovaDbGateway.dataSource()。</li>
 *   <li>    注意 find 而非 get：get 会【回落默认网关】，会让未注册的数据源静默去连默认库，</li>
...
public class DsUtil {
    ...
    public static Connection getConnection(String ds) throws SQLException {
        EovaDbGateway gw = EovaGateways.find(ds);
        if (gw == null) {
    ...
    public static JSONArray getColumnInfoByConfigName(String ds, String tableNamePattern, boolean isView) {
        ...
        try {
            ...
            if (dbType == DbType.oracle) {
                List<EovaRecord> comments = buildOracleComments(ds, tableNamePattern, isView);
                ;
     ...
     * @return
     */
    private static List<EovaRecord> buildOracleComments(String ds, String tableNamePattern, boolean isView) {
        List<EovaRecord> comments = new ArrayList<>();
        try {
            // 获取用户名 String userName = DsUtil.getUserNameByConfigName(ds);  TODO DS 已经相当于owner 条件了 废弃掉 待稳定后删除
            if (isView) {
                // 获取视图的SQL
                String viewSql = EovaGateways.get(ds).queryStr("select text from user_views where view_name = ?", tableNamePattern);
                ...
                for (Name name : tableNames) {
                    ...
                    comments.addAll(EovaGateways.get(ds).find(sql, name.getName()));
                }
            } else {
                // 获取当前表的所有字段的注释
                String sql = "select column_name,comments from user_col_comments where table_name = ?";
    ...
    public static String getTableComment(String ds, String table) {
        ...
        String tableSchema = getSchemaPattern(ds);
        return EovaGateways.queryStr("select TABLE_COMMENT FROM information_schema.TABLES WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?", tableSchema, table);
    }
     ...
     * @return
     */
    private static String getOracleRemark(List<EovaRecord> comments, String en) {
        for (EovaRecord o : comments) {
