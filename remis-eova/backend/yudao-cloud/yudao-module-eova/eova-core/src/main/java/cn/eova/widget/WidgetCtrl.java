...
/**
 * <p>ported from: cn.eova.widget.WidgetCtrl
 * <br>source revision: meta-eova/eova 1b1d39e7350f7e031b216aad0399fc8cc55dce08
 * <br>本单元为逐行等价 port：文件体与旧实现逐字节一致，仅新增本追溯头。
 * <br><b>刻意保留的既有语义：</b>
 * <ol>
 *   <li>通用控件控制器（590 行）：下拉/查找/组合控件的服务端数据</li>
 * </ol>
 */
/**
 * EOVA 控件
...
public class WidgetCtrl extends BaseController {
    ...
    public void find() throws Exception {
        ...
        if (!x.isEmpty(async) && !EovaExp.isFormNull(se.from)) {
            ...
            EovaPage<EovaRecord> page = EovaGateways.get(se.ds).paginate(pageNumber, pageSize, se.getSelect(), fromSql, paras.toArray());
            // 命中唯一值, 直接带回
            if (page.getTotalRow() == 1) {
                EovaRecord e = page.getList().get(0);
                renderJson(LegacyRet.ok().set("pk", e.get(se.pk)).set("cn", e.get(se.cn)).set("data", e));
                return;
            }
        ...
        setAttr("action", url);
        // 用于Grid呈现
        setAttr("objectJson", LegacyJsonKit.toJson(mo));
        setAttr("fieldsJson", LegacyJsonKit.toJson(mfs));
        // 用于query条件
    ...
    public void findJson() {
        ...
        try {
            ...
            EovaPage<EovaRecord> page = EovaGateways.get(se.ds).paginate(pageNumber, pageSize, se.getSelect(), sql, paras.toArray());
    ...
    public void findCnByEn() {
        ...
        List<EovaRecord> txts = EovaGateways.get(ds).find(sql, x.toArray(paras));
        // 没有翻译值，直接返回原值
        if (x.isEmpty(txts)) {
        ...
        json.put("data", LegacyJsonKit.toJson(txts));// 翻译字典数据
        renderJson(json.toJSONString());
    }
    ...
    public void comboJson() {
        ...
        try {
            List<EovaRecord> list = getFieldData(objectCode, en, exp);
     ...
     * @throws Exception
     */
    private List<EovaRecord> getData(EovaOption option, List<Object> paras) throws Exception {
...
//        EovaOption option = EovaOption.dao.getByCode(optionCode);
        ...
        // 构建表达式
        EovaExpBuilder eeb = new EovaExpBuilder(this.getUser(), option);
//        EovaExp se = eeb.build();
        // 获取表达式动态参数
//        List<Object> paras = DynamicParse.buildExpPara(exp); //eeb.getExpParas();
        ...
        List<EovaRecord> list = null;
        if (x.isEmpty(cache)) {
            list = EovaGateways.get(se.ds).find(sql, paras.toArray());
        } else {
            list = EovaGateways.get(se.ds).findByCache(cache, sql, sql, paras.toArray());
        }
     ...
     * @throws Exception
     */
    private List<EovaRecord> getFieldData(String objectCode, String en, String exp) throws Exception {
        // 构建表达式
        ...
        List<EovaRecord> list = null;
        if (x.isEmpty(cache)) {
            list = EovaGateways.get(se.ds).find(sql, x.toArray(paras));
        } else {
            list = EovaGateways.get(se.ds).findByCache(cache, sql, sql, paras.toArray());
        }
...
//            String type = get("type", "combo");
            ...
            List<EovaRecord> list = getData(option, paras);
            renderJson(LegacyRet.data(list).setOk());
...
//            }
    ...
    public void text() {
        ...
        List<EovaRecord> list = new ArrayList<>();
        // 查询需要翻译的数据集(可能为空)
        if (!x.isEmpty(value)) {
...
//		}
        ...
        LegacyRet ret = LegacyRet.ok().set("data", list).set("field_val", se.pk).set("field_txt", se.cn);
