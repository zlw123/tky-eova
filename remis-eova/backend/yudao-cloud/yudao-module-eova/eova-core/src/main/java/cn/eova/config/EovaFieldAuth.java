...
/**
 * <p>ported from: cn.eova.config.EovaFieldAuth
 * <br>source revision: meta-eova/eova 1b1d39e7350f7e031b216aad0399fc8cc55dce08
 * <br>本单元为逐行等价 port：文件体与旧实现逐字节一致，仅新增本追溯头。
 * <br><b>刻意保留的既有语义：</b>
 * <ol>
 *   <li>字段权限表缓存；Db.use(Ds.EOVA).find(sql) -> EovaGateways.get(Ds.EOVA).find(sql)</li>
 *   <li>fieldAuths 为静态 Map<Integer, List<Record>>，属进程内缓存 —— 原样保留</li>
 * </ol>
 */
/**
 * 字段授权
...
public class EovaFieldAuth {
     ...
     * 租户(企业)字段授权
     */
    public static Map<Integer, List<EovaRecord>> fieldAuths = new HashMap<>();
    ...
    public static void authReload(Integer companyId) {
        ...
        // 加载全部
        if (companyId == 0) {
            List<EovaRecord> fieldAuth = EovaGateways.get(Ds.EOVA).find(sql);
    ...
    public static Set<String> getDisableFields(User user) {
        ...
        if (!fieldAuths.isEmpty()) {
            List<EovaRecord> companyAuths = fieldAuths.get(user.getCompanyId());
            if (!x.isEmpty(companyAuths)) {
                for (EovaRecord ca : companyAuths) {
