// ported from: meta-eova/eova/core/src/main/java/cn/eova/engine/EovaExp.java
// old FQCN: cn.eova.engine.EovaExp
/**
...
public class EovaExp {
    ...
    private static final Logger log = LoggerFactory.getLogger(EovaExp.class);
    ...
    public EovaExp(EovaOption ops) {
        try {
            ...
            log.error("Eova MetaExp 解析异常:" + ops.getSql());
        }
    }

    public EovaExp(String ds, String exp) {
        try {
            // 无参
//            if (!exp.contains(";")) {
//                this.sql = exp;
...
//            }
    ...
    private void buildField() {
        ...
        try {
            ...
            log.debug("EovaExp initParse Select Item Error:" + sp.getSelectItem());
        }
    }
    ...
    public static MetaField buildItem(int index, String en, String cn, boolean isQuery, EovaOption option) {
        ...
        try {
            if (option != null) {
                Map<String, Object> fields = option.getConfObj("field_width");
                if (fields != null) {
                    for (Object key : fields.keySet()) {
                        if (key.equals(en)) {
                            Object raw = fields.get(key);
                            width = raw instanceof Number ? ((Number) raw).intValue() : Integer.parseInt(String.valueOf(raw));
                            break;
                        }
                    }
                }
