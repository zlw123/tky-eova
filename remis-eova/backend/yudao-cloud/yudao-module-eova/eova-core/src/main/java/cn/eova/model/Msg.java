...
/**
 * <p>ported from: cn.eova.model.Msg
 * <br>source revision: meta-eova/eova 1b1d39e7350f7e031b216aad0399fc8cc55dce08
 * <br>本单元为逐行等价 port：文件体与旧实现逐字节一致，仅新增本追溯头。
 * <br><b>刻意保留的既有语义：</b>
 * <ol>
 *   <li>站内消息的已读/清理 SQL 与条件顺序属契约</li>
 * </ol>
 */
/**
 * 系统消息
...
public class Msg extends BaseModel<Msg> {
    ...
    public void read(int type, int toUid, int refId) {
        gw(Ds.EOVA).update("update eova_msg set status = 1 where type = ? and to_uid = ? and ref_id = ?", type, toUid, refId);
    }
    ...
    public void deleteNoRead(int type, int refId) {
        gw(Ds.EOVA).delete("delete from eova_msg where status = 0 and type = ? and ref_id = ?", type, refId);
    }
    ...
    public void deleteByRefId(int refId) {
        gw(Ds.EOVA).delete("delete from eova_msg where ref_id = ?", refId);
    }
