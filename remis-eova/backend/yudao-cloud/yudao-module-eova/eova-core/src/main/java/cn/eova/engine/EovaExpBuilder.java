...
/**
 * <p>ported from: cn.eova.engine.EovaExpBuilder
 * <br>source revision: meta-eova/eova 1b1d39e7350f7e031b216aad0399fc8cc55dce08
 * <br>本单元为逐行等价 port：文件体与旧实现逐字节一致，仅新增本追溯头。
 * <br><b>刻意保留的既有语义：</b>
 * <ol>
 *   <li>表达式构建器（224 行）：把 eova_field.exp 的配置解析为 SQL/固定项</li>
 *   <li>【已声明适配 1】com.jfinal.kit.JsonKit -> LegacyJsonKit（R37：enjoy 无 JsonKit）</li>
 *   <li>【已声明适配 2】com.jfinal.kit.Kv -> LegacyKv（R37/R40）</li>
 *   <li>【已声明适配 3】Db.use(ds) -> EovaGateways.get(ds)；Record -> EovaRecord</li>
 * </ol>
 */
/**
 * Eova表达式构建器
...
public class EovaExpBuilder {
    ...
    public EovaExpBuilder(User user, EovaOption option) {
        this(option.getSql());
        this.user = user;
        this.ds = option.getDs();
    }
    ...
    public EovaExp build() {
        //  TODO 复杂表达式构建,待重构.... 先保障 eova_option 逻辑通畅.
        // 解析表达式.
        String sql = buildExp();
        return new EovaExp(ds, sql);
    }
    ...
    private String parseExp() {
        ...
        // 动态解析(主要是获取用户域数据)
        return ExpUtil.parseSql(exp, LegacyKv.of("user", this.user));
    }
...
//            parseConfigExp();
//            return parseExp();
            x.log.error("Exp为空:" + LegacyJsonKit.toJson(this));
        }
     ...
     * @return
     */
    public List<EovaRecord> buildFixedItem() {
        if (x.isEmpty(exp)) {
            return null;
        }
        ...
        if (isFixedItem()) {
            ...
            for (String s : ss) {
                String[] item = s.split("=");
                arrs.add(new EovaRecord().set("id", item[0]).set("cn", item[1]));
            }
        ...
        // 根据表达式取值 构建数据 (比如选择控件)
        EovaExp se = build();
        return EovaGateways.get(se.ds).find(se.sql);
    }
