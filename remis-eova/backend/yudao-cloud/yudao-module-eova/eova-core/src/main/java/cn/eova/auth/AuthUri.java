 ...
 * <br><b>刻意保留的既有语义：</b>
 * <ol>
 *   <li>鉴权 URI 规则（292 行）：按菜单/按钮生成受控 URL 集合</li>
 *   <li>【已声明适配 1】com.jfinal.kit.Kv -> LegacyKv（R37/R40）</li>
 *   <li>【已声明适配 2】Db.use(Ds.EOVA) -> EovaGateways.get(Ds.EOVA)；Record -> EovaRecord</li>
 *   <li>com.jfinal.template.Engine 为 enjoy 制品，沿用</li>
 *   <li>【已声明 stub 依赖】只读 cn.eova.config.EovaConfig.EOVA_INDEX（旧源码 :90，逐字一致）</li>
 * </ol>
 */
/**
 * <pre>
...
//        loginAuth.add("/api/table/width");                  // 更新表列宽
    ...
    public static void build(User user) {
        ...
        List<EovaRecord> btns = EovaGateways.get(Ds.EOVA).find(sql, paras.toArray());
    ...
    public static void buildUriByAuthCode(Set<String> auths, String auth, Menu menu) {
        LegacyKv kv = LegacyKv.of("menu", menu).set("conf", menu.getMenuConfig());
        // 运行时动态调试菜单权限生成结果
     ...
     * @return
     */
    public static String parseAuthUri(String s, LegacyKv pms) {
        if (x.isEmpty(s)) {
            return s;
        }
