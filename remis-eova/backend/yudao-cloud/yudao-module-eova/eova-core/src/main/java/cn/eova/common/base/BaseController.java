...
/**
 * <p>ported from: cn.eova.common.base.BaseController
 * <br>source revision: meta-eova/eova 1b1d39e7350f7e031b216aad0399fc8cc55dce08
 * <br>本单元为逐行等价 port：文件体与旧实现逐字节一致，仅新增本追溯头。
 * <br><b>刻意保留的既有语义：</b>
 * <ol>
 *   <li>所有 EOVA 控制器的基类（363 行）—— W4 批量 port 的前置</li>
 *   <li>【已声明适配 1】com.jfinal.core.Controller -> cn.eova.compat.jfinal.core.LegacyController</li>
 *   <li>【已声明适配 2】com.jfinal.core.NotAction -> cn.eova.compat.jfinal.core.LegacyNotAction</li>
 *   <li>    （第 59 轮新增接缝：@NotAction 方法不得映射成 Action，否则 UID()/RID() 会被暴露成 URL）</li>
 *   <li>【已声明适配 3】com.jfinal.render.TemplateRender -> LegacyTemplateRender</li>
 *   <li>【已声明适配 4】com.jfinal.json.Json.getJson().parse(s, Ret.class) -> LegacyJsonKit.parse(s, LegacyRet.class)</li>
 *   <li>    （与 LoginService 的适配 3 同构，已由 LegacyKvRetGoldenTest 对照旧制品验证）</li>
 *   <li>【已声明适配 5】com.jfinal.plugin.activerecord.Record -> cn.eova.db.EovaRecord</li>
 *   <li>【已声明适配 6】com.jfinal.plugin.activerecord.Model -> cn.eova.db.EovaModel</li>
 *   <li>getModel 用 modelClass.newInstance() + set(field,val) 的反射式填充，属既有语义</li>
 *   <li>render(String) 里 /eova/ -> /eova/_view/ 的路径改写属【页面契约】，不得简化</li>
 * </ol>
 */
public class BaseController extends LegacyController {
...
//            x.conf.getBool("devMode", false)
    ...
    protected List<EovaRecord> getJsonToRecords(String name) {
     ...
     * @return
     */
    protected List<EovaRecord> getRecords() {
        JSONArray list = (JSONArray) getJson();
        return RecordUtil.parseArray(list.toJSONString());
    }

    /**
     * 获取提交的JSON 转 LegacyRet
     * @return
     */
    protected LegacyRet getJsonToRet() {
        String s = getRawData();
        if (x.isEmpty(s)) {
            return null;
        }

        return LegacyJsonKit.parse(s, LegacyRet.class);
    }

    /**
     * 获取提交的JSON 转 EovaRecord
     * @return
     */
    @LegacyNotAction
    public EovaRecord getJsonToRecord() {
        ...
        // 将 JSONObject 的所有键值对添加到 EovaRecord 中
     ...
     * @return
     */
    protected <T extends EovaModel> T getModel(Class<? extends EovaModel> modelClass, String... fields) {
        EovaModel<?> m = null;
        try {
    ...
    public void renderEnjoy(String view) {
        render(new LegacyTemplateRender(view));
    }
