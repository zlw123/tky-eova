...
/**
 * <p>ported from: cn.eova.model.MetaField
 * <br>source revision: meta-eova/eova 1b1d39e7350f7e031b216aad0399fc8cc55dce08
 * <br>本单元为逐行等价 port：文件体与旧实现逐字节一致，仅新增本追溯头。
 * <br><b>刻意保留的既有语义：</b>
 * <ol>
 *   <li>元字段模型（588 行）：字典/表达式/校验/DIY 字段，EovaMeta 的核心元数据单元</li>
 *   <li>本单元取代此前的【已声明 compile-stub】MetaField</li>
 *   <li>【已声明适配 1】com.jfinal.kit.Kv -> LegacyKv（R37/R40）</li>
 *   <li>【已声明适配 2】Db.use(Ds.EOVA) -> EovaGateways.get(Ds.EOVA)；Record -> EovaRecord</li>
 *   <li>getItems/getDiyFields 的 SQL 与参数顺序属契约，不得调整</li>
 * </ol>
 */
/**
 * 字段属性
...
//	}
     ...
     * @return
     */
    public List<EovaRecord> getItems() {
    ...
    public void deleteByObjectId(int objectId) {
        ...
        EovaGateways.get(Ds.EOVA).update(sql, objectId);
    }
    ...
    private void buildTableMetaField(String objectCode, User user, List<MetaField> mfs) {
        ...
        // 用户个性化设置
        List<EovaRecord> diyFields = EovaGateways.get(Ds.EOVA).find("select * from eova_diy where type = ? and uid = ? and object_code = ?", DIY_TABLE, user.getId(), objectCode);
    ...
    private void buildQueryMetaField(String objectCode, User user, List<MetaField> mfs) {
        ...
        // 用户个性化设置
        List<EovaRecord> diyFields = EovaGateways.get(Ds.EOVA).find("select * from eova_diy where type = ? and uid = ? and object_code = ?", DIY_QUERY, user.getId(), objectCode);
        for (MetaField f : mfs) {
    ...
    public int updateTableNameByCode(String objectCode, String en, String tableName) {
        return EovaGateways.get(Ds.EOVA).update("update eova_field set table_name = ? where object_code = ? and en = ?", tableName, objectCode, en);
    }
    ...
    public void updateOrderNum(String objectCode, String sen, String ten) {
        ...
        EovaGateways.get(Ds.EOVA).update("update eova_field set num = ? where object_code = ? and en = ?", t.getInt("num") + 1, objectCode, sen);
    }
