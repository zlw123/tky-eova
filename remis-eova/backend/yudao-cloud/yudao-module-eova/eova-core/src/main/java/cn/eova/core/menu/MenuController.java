...
/**
 * <p>ported from: cn.eova.core.menu.MenuController
 * <br>source revision: meta-eova/eova 1b1d39e7350f7e031b216aad0399fc8cc55dce08
 * <br>本单元为逐行等价 port：文件体与旧实现逐字节一致，仅新增本追溯头。
 * <br><b>刻意保留的既有语义：</b>
 * <ol>
 *   <li>菜单控制器（782 行）：菜单树/排序/增删改（EOVA 元数据驱动的最大控制器之一）</li>
 *   <li>【已声明适配】Before/Kv/Ret/Tx/TxConfig/NestedTransactionHelpException/Db/Record 全部走既有接缝</li>
...
public class MenuController extends BaseController {
    ...
    public void props() {
        ...
        renderJson(LegacyRet.data(props).setOk());
    }
    ...
    public void updateLayerSize() {
        ...
        LegacyKv kv = menu.getMenuConfig();
...
//            new ButtonFactory(TemplateConfig.SINGLE_GRID, menuCode, config).build();
    ...
    public void doExport() {
        ...
        List<EovaRecord> list = EovaGateways.get(Ds.EOVA).find(sql);
        // 删除语句
        list.forEach(x -> {
        ...
        for (EovaRecord r : list) {
            List<EovaRecord> btns = EovaGateways.get(Ds.EOVA).find("select * from eova_button where menu_code = ?", r.getStr("code"));
    ...
    @LegacyTxConfig(Ds.EOVA)
    public void add() {
        EovaRecord data = getJsonToRecord();
...
//                // 默认指定通用SQL查询
//                EovaGateways.get(Ds.EOVA).update("update eova_object set biz_intercept = 'com.eova.aop.impl.SqlQueryIntercept' where code = ?", objectCode);
//            } else {
//            }
            menu.set("type", type);
...
//			menu.setConfig(config);
            // LegacyKv menuConfig = menu.getMenuConfig();
            menu.save();
...
//			renderJson(new Easy());
            ...
            throw new LegacyNestedTransactionHelpException("新增菜单异常");
        }
...
//
//        List<EovaRecord> auths = RoleBtn.dao.findByMenuId(id);
//        Menu menu = Menu.dao.findById(id);
//
//        setAttr("menu", menu);
//        setAttr("btnMap", btnMap);
...
//        setAttr("auths", auths);
        ...
        LegacyRet ret = LegacyRet.ok().set("menu", menu).set("btns", btns).set("auths", auths).set("roles", roles);
        renderJson(ret);
    }
    ...
    public void initEovaButton() {
...

        {
            ...
            EovaGateways.get(Ds.EOVA).update(sql, "eova_menu", "新增");
        }
        {
            String sql = "UPDATE eova_button SET is_hide = 1 WHERE menu_code = ? and name = ?";
            EovaGateways.get(Ds.EOVA).update(sql, "eova_menu", "查看");
        }
        {
            String sql = "UPDATE eova_button SET is_hide = 1 WHERE menu_code = ? and name = ?";
            EovaGateways.get(Ds.EOVA).update(sql, "eova_task", "导入");
        }
        {
            String sql = "UPDATE eova_button SET is_hide = 1 WHERE menu_code = ? and name = ?";
            EovaGateways.get(Ds.EOVA).update(sql, "eova_object", "新增");
        }
        {
            String sql = "UPDATE eova_button SET is_hide = 1 WHERE menu_code = ? and name = ?";
            EovaGateways.get(Ds.EOVA).update(sql, "eova_object", "查看");
        }
        {
            String sql = "UPDATE eova_button SET is_hide = 1 WHERE menu_code = ? and name = ?";
            EovaGateways.get(Ds.EOVA).update(sql, "eova_object", "新增");
        ...
        {
            String sql = "UPDATE eova_button SET is_hide = 1 WHERE menu_code = ? and name = ?";
            EovaGateways.get(Ds.EOVA).update(sql, "sys_auth_users", "查看");
        }
        {
            String sql = "UPDATE eova_button SET is_hide = 1 WHERE menu_code = ? and name = ?";
            EovaGateways.get(Ds.EOVA).update(sql, "sys_auth_users", "用户详细信息新增");
        }
        {
            String sql = "UPDATE eova_button SET is_hide = 1 WHERE menu_code = ? and name = ?";
            EovaGateways.get(Ds.EOVA).update(sql, "sys_auth_users", "用户详细信息删除");
        }
        {
            String sql = "UPDATE eova_button SET is_hide = 1 WHERE menu_code = ? and name = ?";
            EovaGateways.get(Ds.EOVA).update(sql, "sys_auth_role", "查看");
        }
        {
            String sql = "UPDATE eova_button SET is_hide = 1 WHERE menu_code = ? and name = ?";
            EovaGateways.get(Ds.EOVA).update(sql, "sys_auth_role", "导入");
        }
        {
            String sql = "UPDATE eova_button SET is_hide = 1 WHERE menu_code = ? and ui <> 'query'";
            EovaGateways.get(Ds.EOVA).update(sql, "sys_log");
        }
    ...
    public void initOracleType() {
...

        {
            ...
            EovaGateways.get(Ds.EOVA).update(sql);
        }
        {
            // NUMBER
    ...
    public void doFlow() {
        ...
        try {
            ...
            throw new LegacyNestedTransactionHelpException("创建流程失败");
        }
...
//        new ButtonFactory(TemplateConfig.SINGLE_GRID, menuCode, config).build();
        ...
        // 默认干掉除查询和查看的按钮
        EovaGateways.get(Ds.EOVA).update("update eova_button set is_hide = 1 where menu_code = ? and name not in ('查询', '查看')", menuCode);

        // 获取查询和查看的ID, 用于自动分配
        List<Integer> ids = EovaGateways.get(Ds.EOVA).query("select id from eova_button where menu_code = ? and name in ('查询', '查看')", menuCode);
