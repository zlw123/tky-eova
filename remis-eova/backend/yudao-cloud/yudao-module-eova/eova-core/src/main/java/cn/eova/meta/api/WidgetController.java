...
/**
 * <p>ported from: cn.eova.meta.api.WidgetController
 * <br>source revision: meta-eova/eova 1b1d39e7350f7e031b216aad0399fc8cc55dce08
 * <br>本单元为逐行等价 port：文件体与旧实现逐字节一致，仅新增本追溯头。
 * <br><b>刻意保留的既有语义：</b>
 * <ol>
 *   <li>API 控件控制器（288 行）：/api/widget 的数据与翻译入口</li>
 * </ol>
 */
/**
 * EOVA 控件
...
//        List<Object> paras = DynamicParse.buildExpPara(exp); //eeb.getExpParas();
        ...
        if (x.isEmpty(cache)) {
            list = EovaGateways.get(option.getDs()).find(sql, paras.toArray());
        } else {
            list = EovaGateways.get(option.getDs()).findByCache(cache, sql, sql, paras.toArray());
        }
