...
/**
 * <p>ported from: cn.eova.service.LoginService
 * <br>source revision: meta-eova/eova 1b1d39e7350f7e031b216aad0399fc8cc55dce08
 * <br>本单元为逐行等价 port：文件体与旧实现逐字节一致，仅新增本追溯头。
 * <br><b>刻意保留的既有语义：</b>
 * <ol>
 *   <li>登录/会话服务（472 行）：账号校验、会话建立与失效、登录缓存</li>
 *   <li>【已声明适配 1】com.jfinal.plugin.ehcache.CacheKit -> cn.eova.compat.cache.LegacyCacheKit</li>
...
public class LoginService {
    ...
    private User findUserByUid(Object uid) {
        EovaRecord r = EovaGateways.get(userDs).findFirst(String.format("select * from %s where %s = ?", userTable, userId), xx.ET(uid));
        return initUser(r);
    }
     ...
     * @return
     */
    private EovaRecord findByLoginId(String loginId) {
        return EovaGateways.get(userDs).findFirst(String.format("select * from %s where %s = ?", userTable, userAccount), loginId);
    }
     ...
     * @return
     */
    private EovaRecord findUserByAccount(String account) throws Exception {
        ...
        if (!x.isEmpty(loginServer)) {
            ...
            if (!x.isEmpty(val)) {
                ...
                EovaRecord user = null;
                try {
                    String sql = String.format("select * from %s where %s = ?", userTable, serverUid);
                    user = EovaGateways.get(userDs).findFirst(sql, val);
                } catch (Exception e) {
                    e.printStackTrace();
        ...
        return EovaGateways.get(userDs).findFirst(sql + collect, paras.toArray());
    }
     ...
     * @return
     */
    private LegacyRet loginValidate(String loginId, String loginPwd) {
        EovaRecord r = null;
        try {
            r = findUserByAccount(loginId);
        } catch (Exception e) {
            return LegacyRet.fail(e.getMessage());
        }
        if (r == null) {
            return LegacyRet.fail("用户名不存在");
        }
        // 加密方式可配置
        ...
        if (x.isEmpty(userPassword) || !userPassword.equals(loginPwd)) {
            return LegacyRet.fail("密码错误");
        }
        // 初始化用户, 并回登录源数据
        return LegacyRet.ok().set(USER, initUser(r));
     ...
     * @return
     */
    public User initUser(EovaRecord r) {
        User user = new User();
    ...
    public void forceLogout(String uid) {
        ...
        if (session != null) {
            ...
            xx.info("销毁会话成功:" + sid);
            LegacyCacheKit.remove(BaseCache.LOGIN, sid);
            deleteSession(uid);
        }
    }
    ...
    public void forceLogoutByLoginId(String loginId) {
        EovaRecord user = findByLoginId(loginId);
     ...
     * @return
     */
    public LegacyRet login(String loginId, boolean keepLogin) {
//		String userDs = x.conf.get("login.user.ds", Ds.EOVA);
//		String userTable = x.conf.get("login.user.table", "eova_user");
//		String userId = x.conf.get("login.user.id", "id");
        ...
        // 保存登录 session 到数据库
        String sid = LegacyStrKit.getRandomUUID();
        Session session = new Session(sid, user.getId(), expire);
        if (!session.save()) {
            return LegacyRet.fail("会话保存异常，请联系管理员");
        }

        user.put(SID, sid); // SID备用

        LegacyCacheKit.put(BaseCache.LOGIN, sid, user);

        return LegacyRet.ok().set(USER, user).set("maxAgeInSeconds", maxAgeInSeconds);
    }
    ...
    public LegacyRet login(String loginId, String loginPwd, boolean keepLogin, String ip) {
        ...
        LegacyRet ret = loginValidate(loginId, loginPwd);
        if (ret.isFail()) {
            return ret;
        }
    ...
    public User getLoginUser(String sid) {
        return LegacyCacheKit.get(BaseCache.LOGIN, sid);
    }

    private void loginLog(Object uid, String ip) {
        EovaRecord loginLog = new EovaRecord().set("user_id", uid).set("ip", ip);
        EovaGateways.get(Ds.EOVA).save("eova_login_log", loginLog);
    }
    ...
    //		// 将来可能把 user 用 id : obj 的形式放缓存，更新缓存只需要 LegacyCacheKit.remove("account", id) 就可以了，
    ...
    public void initAuth(User user) {
        ...
        List<EovaRecord> bss = EovaGateways.get(Ds.EOVA).find(sql, paras.toArray());
        for (EovaRecord r : bss) {
    ...
    private void deleteSession(String uid) {
        EovaGateways.get(Ds.EOVA).delete("DELETE FROM eova_session where user_id = ?", uid);
    }
}
