...
/**
 * <p>ported from: cn.eova.mod.EovaModUtil
 * <br>source revision: meta-eova/eova 1b1d39e7350f7e031b216aad0399fc8cc55dce08
 * <br>本单元为逐行等价 port：文件体与旧实现逐字节一致，仅新增本追溯头。
 * <br><b>刻意保留的既有语义：</b>
 * <ol>
 *   <li>模块工具（285 行）：Mod 安装/升级/卸载时执行 SQL 脚本、生成 ESQL、加载 Mod 类加载器</li>
 *   <li>【已声明适配 1】Db.use(ds).update(...) -> EovaGateways.get(ds).update(...)</li>
 *   <li>【已声明适配 2】Db.use(ds).batch(sqls, sqls.size()) -> EovaGateways.get(ds).batch(...)（第 64 轮给网关补的 batch(List,int)，语义按 jfinal DbPro 字节码：分块提交、结果压平）</li>
 *   <li>【已声明适配 3】com.jfinal.plugin.activerecord.Db（import 去除，改用 EovaGateways）</li>
 *   <li>静态字段 modLoader 由 EovaConfig 赋值，属宿主装配点</li>
 * </ol>
 */
public class EovaModUtil {
    ...
    public static void executeSql(String ds, String sqlFile) {
        ...
        for (String sql : sqls) {
            ...
            EovaGateways.get(ds).update(sql.trim());
        }
    }
    ...
    public static void executeSql(String ds, String group, String code, String fileName) {
        ...
        for (String sql : sqls) {
            System.out.println(sql);
            EovaGateways.get(ds).update(sql);
        }
    }
    ...
    public static List<String> executeEsql(String ds, String sqlPath) {
        ...
        EovaGateways.get(ds).batch(sqls, sqls.size());
